...
var loadMediaSelector = function loadMediaSelector() {
  $('[data-plugin="media"]').each(function (index, item) {
    var options = $(item).data('options');
    var id = $(item).attr('id');
    $(item).replaceWith("\n            <div class=\"media-item\">\n                <input type=\"hidden\" name=\"".concat(options.name, "[key]\" value=\"").concat(options.name, "\">\n                <div id=\"").concat(id, "\" data-options='").concat(JSON.stringify(options), "' class=\"file-viewer open-media-dialog\">").concat(options.placeholder, "</div>\n                <input type=\"hidden\" name=\"").concat(options.name, "[path]\" value=\"\">\n                <input type=\"hidden\" name=\"").concat(options.name, "[id]\" value=\"\">\n                <div class=\"delete-file atm-none\" data-options='").concat(JSON.stringify(options), "'><i class=\"material-icons mdc-button__icon\">delete</i></div>\n                <div class=\"file-name atm-none\"></div>\n            </div>\n        "));
...
var mediaManager = function mediaManager(refId, id, file) {
  if (config.mime_types['image/*'].split(',').find(function (x) {
    return x === '.' + file.split('.').pop();
  })) {
    $('#' + refId).html('<img src="' + file + '">');
  } else if (config.mime_types['video/*'].split(',').find(function (x) {
    return x === '.' + file.split('.').pop();
  })) {
    $('#' + refId).html('<img src="' + asset + 'atriatech/media/extra/icons/svg/035-file-7.svg' + '">');
  } else if (config.mime_types['audio/*'].split(',').find(function (x) {
    return x === '.' + file.split('.').pop();
  })) {
...
$('body').delegate('.open-media-dialog', 'click', function () {
  ...
  if (inputOptions.accept !== undefined) {
    if (config.mime_types[inputOptions.accept] !== undefined) {
      ...
      var _iterator = _createForOfIteratorHelper(inputOptions.accept.split(',')),
        _step;
      try {
...
function loadMediaSelectorWithJS(id, options) {
  var noContainer = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  var html = '';
  if (!noContainer) {
    html += '<div class="media-container">';
  }
  html += "<div id=\"".concat(id, "\" data-plugin=\"media\" data-options='").concat(JSON.stringify(options), "'></div>");
  if (!noContainer) {
    html += '</div>';
  }
  $('#' + id).replaceWith(html);
...
module.exports = {
  loadMediaSelectorWithJS: loadMediaSelectorWithJS
};
...
/******/ 	var __webpack_exports__ = __webpack_require__("./src/assets/js/selector.js");
/******/ 	AtriatechMedia = __webpack_exports__;
/******/ 	
/******/ })()
;