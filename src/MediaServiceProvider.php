...
class MediaServiceProvider extends ServiceProvider
{
	...
	public function boot()
	{
		...
		Blade::directive('ar7_media', function ($file) {
			$file = trim($file, "'");
			if ($file == 'css') {
				return '<?php $css_media_selector = asset(trim(config(\'ar7_media.url_prefix\'), \'/\') . \'/\' . \'atriatech/media/css/media-selector.css\'); echo \'<link href="\' . $css_media_selector . \'?ver=\' . rand(10000000, 99999999) . \'" rel="stylesheet" type="text/css"/>\'; ?>';
			} else if ($file == 'js') {
				return '<?php $js_media_selector = asset(trim(config(\'ar7_media.url_prefix\'), \'/\') . \'/\' . \'atriatech/media/js/media-selector.js\'); echo \'<script src="\' . route(\'ar7_media_router\') . \'"></script><script src="\' . route(\'ar7_media_config\') . \'"></script><script src="\' . $js_media_selector . \'?ver=\' . rand(10000000, 99999999) . \'"></script>\'; ?>';
			} else {
				return '';
			}
		});
