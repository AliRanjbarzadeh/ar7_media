...
class MediumHelper
{
	...
	{
		$path = trim($path, '/');
		$path = str_replace('public/' . trim(config('ar7_media.upload_folder'), '/'), '', $path);
		$path = trim($path, '/');
		if (!empty($path)) {
			$path = 'public/' . trim(config('ar7_media.upload_folder'), '/') . '/' . $path . '/';
		} else {
			$path = 'public/' . trim(config('ar7_media.upload_folder'), '/') . '/';
		}
	...
	{
		return config('ar7_media.medium_model') ?? Medium::class;
	}
}